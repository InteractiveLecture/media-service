...
func TestParseRange(t *testing.T) {
	...
	ranges, err := ParseRangeRequest(testcase, 1000)
	if err != nil {
		t.Error(err)
	}
	if len(ranges) != 3 {
	...
	expectedResults := []Range{Range{0, 100}, Range{800, 999}, Range{0, 900}}
	if !compareAll(expectedResults, ranges) {
		t.Fatalf("expected %v, got %v", expectedResults, ranges)
	}
	assert.Equal(t, int64(101), expectedResults[0].ContentLength())
	assert.Equal(t, int64(901), expectedResults[2].ContentLength())
